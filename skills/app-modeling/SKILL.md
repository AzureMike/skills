...
8. Generate the Bicep using [naming-conventions.md](references/naming-conventions.md), compile it with an extension compatible with the exact target contract, then run the host validator backed by `scripts/check.mjs`. The checker denies compiler diagnostics, unresolved symbolic resources, anything other than one unconditional `Radius.Core/applications` resource, and invalid literal container-map keys. Treat `ERROR` as an unavailable check, not a pass. Never remove a required backend activation, native configuration value, secret binding, or dependency edge to obtain `ALLOW`.
...
| Container key in `containers` map | Service short name as a lowercase RFC 1123 label of at most 63 characters (e.g., `api-worker`) |
...
- [ ] The host validator backed by `scripts/check.mjs` returns `ALLOW`: no compiler diagnostics, symbolic compiled resources, exactly one unconditional `Radius.Core/applications@2025-08-01-preview`, and valid literal container-map keys. `extension radius` is always declared; a local custom-types extension (for example `extension customTypes`) is also declared when the app uses a generated `Radius.Resources/*` custom type. No per-namespace or per-type extensions.
